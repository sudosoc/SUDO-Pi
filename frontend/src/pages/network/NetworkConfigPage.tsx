...
import { Globe, ShieldBan, Shuffle } from "lucide-react";
...
const TABS = [
  { id: "dns",      label: "DNS & DHCP",     icon: Globe     },
  { id: "control",  label: "Device Control", icon: ShieldBan },
  { id: "ports",    label: "Port Forwards",  icon: Shuffle   },
] as const;
...
export default function NetworkConfigPage() {
  ...
  const tab = (params.get("tab") ?? "dns") as TabId;
  const valid = TABS.some((t) => t.id === tab);
  const active = valid ? tab : "dns";
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto overflow-x-hidden">
        <Suspense fallback={<TabLoader />}>
          {active === "dns"     && <DnsPage />}
          {active === "control" && <DeviceControlPage />}
          {active === "ports"   && <PortForwardPage />}
        </Suspense>
      </div>
    </div>
  );
